...
st.markdown(
    "Upload intraday CSV (2 years OK). App detects basic columns and creates daily aggregates, histograms, orderflow metrics, True Range forecast, and quantile forecasts."
)
    ...
    if datetime_col and datetime_col in df.columns:
        ...
        for cand in ["datetime","timestamp","time","date"]:
            if cand in df.columns:
                ...
                datetime_col = cand
                break
    if datetime_col is None or df[datetime_col].isna().all():
...
def to_daily_ohlc(df, datetime_col, price_cols):
    ...
    df = df.dropna(subset=[datetime_col] + list(price_cols.values()))
...
def true_range_forecast(daily):
    ...
    pred = model.predict(start=len(ts), end=len(ts))
    return float(pred[0])

# ---------- UI Inputs ----------
uploaded = st.file_uploader("Upload intraday CSV", type=['csv'])
if uploaded is None:
    st.info("Upload a CSV with datetime, open, high, low, close. Optional: buy_volume & sell_volume OR tick_direction & volume for orderflow.")
    st.stop()
...
st.metric('Expected True Range %', f"{tr_forecast:.2f}" if not np.isnan(tr_forecast) else "N/A")
...
if len(quantile_list) > 0:
    ...
    st.warning('Select at least one quantile in the sidebar.')
...
if make_report:
    ...
    with zipfile.ZipFile(buf, 'w') as zf:
        ...
        fig, ax = plt.subplots()
        daily['high'].plot(kind='hist', bins=50, ax=ax)
        ax.set_title('High distribution')
        imgbuf = io.BytesIO()
        ...
        fig, ax = plt.subplots()
        daily['low'].plot(kind='hist', bins=50, ax=ax)
        ax.set_title('Low distribution')
        imgbuf = io.BytesIO()
        plt.savefig(imgbuf, format='png')
        plt.close(fig)
...
st.markdown('---')
st.caption('Neemistat — lightweight Streamlit rebrand of QuantiStat. True Range % forecast added.')
